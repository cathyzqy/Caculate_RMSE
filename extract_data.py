...
def prettify_header(metric):
    metrics_to_remove = ['instance', 'job', 'mode', '__name__', 'container', 'endpoint', 'namespace', 'pod',
                         'prometheus', 'service']
    for i in metrics_to_remove:
        if i in metric: del metric[i]
    if len(metric) > 1:
        raise Exception('too many metric labels')
    else:
        return next(iter(metric.keys()))


def export_data_for_prp(sender, receiver, sender_instance, receiver_instance, start_time, end_time, monitor_url,
                        cluster, **params):
    if cluster == 'prp':
        AVG_INT = 1
        query = (
            # 'label_replace((sum by (container)(rate(container_network_transmit_bytes_total{{namespace=~"{3}", interface="{2}", pod=~"{0}.*"}}[{1}m]) * 8)), "network_throughput", "$0", "container", "(.+)") '
        ...
        query = (
            ...
            '').format(sender['name'], AVG_INT, sender['interface'], 'dtnaas', sender_instance, receiver_instance)
        if 'nvmeof' in cluster:
            'label_replace(sum by (instance)(irate(node_network_transmit_bytes_total{instance=~"165.124.33.175:9100.*", device="vlan555"}[15m])), "network_throughput", "$0", "instance", "(.+)") or label_replace(sum by (job)(irate(node_disk_written_bytes_total{instance=~"131.193.183.248:9100.*", device=~"nvme.*"}[15m])),"Goodput", "$0", "job", "(.+)") or label_replace(sum by (job)(1 - irate(node_cpu_seconds_total{mode="idle", instance="165.124.33.175:9100"}[1m])),"CPU", "$0", "job", "(.+)") or label_replace(node_memory_Active_bytes{instance="165.124.33.175:9100"}, "Memory_used", "$0", "instance", "(.+)") or label_replace(sum by (job)(irate(node_disk_written_bytes_total{instance=~"165.124.33.175:9100.*", device=~"nvme.*"}[15m])),"NVMe_from_ceph", "$0", "job", "(.+)") or label_replace(sum by (job)(irate(node_disk_read_bytes_total{instance=~"165.124.33.175:9100.*", device=~"nvme.*"}[15m])),"NVMe_from_transfer", "$0", "job", "(.+)") or label_replace(sum by (job)(irate(node_disk_io_time_seconds_total{instance=~"165.124.33.175:9100.*", device=~"nvme.*"}[15m])),"NVMe_total_util", "$0", "job", "(.+)") or label_replace(sum by (job)(node_network_speed_bytes{instance=~"165.124.33.175:9100.*", device="vlan555"} * 8), "NIC_speed", "$0", "job", "(.+)") or label_replace(sum by (job)(irate(node_netstat_Tcp_RetransSegs{instance=~"165.124.33.175:9100.*"}[15m])), "Packet_losses", "$0", "job", "(.+)") '
            query = query.replace(
                'label_replace(sum by (job)(irate(node_disk_written_bytes_total{instance=~"131.193.183.248:9100.*", device=~"nvme.*"}[15m])),"Goodput", "$0", "job", "(.+)")',
                'label_replace(sum by (job)(irate(node_disk_read_bytes_total{instance=~"165.124.33.175:9100.*", device=~"nvme.*"}[15m])),"Goodput", "$0", "job", "(.+)")')
        ...
        while end_time > start_time:
            ...
            for result in response['data']['result']:
                ...
                data_in_period = df if data_in_period is None else data_in_period.merge(df, how='outer',
                                                                                        on='Time').sort_index()
        ...
        return dataset[cols], dataset['num_workers'][0]
...
def main(transfer_id, cluster):
    if cluster == 'prp':
        ...
        raise Exception('prp or mrp is only supported')
    ...
    ts_data, num = export_data_for_prp(sender, receiver, sender_instance, receiver_instance, result['start_time'],
                                       result['end_time'], monitor, cluster, num_workers=result['num_workers'])
...
if __name__ == '__main__':
    ...
    for i in range(1144, 1145):
        transfer_id = i
        data, num = main(transfer_id, cluster)
        data.to_csv('result/throughput/mrp/real/mrp/' + str(transfer_id) + '.csv')